...
dir.create(file.path(res_dir, "DE_Results[1]"), showWarnings = TRUE)
dir.create(file.path(res_dir, "DE_Volcanos[1]"), showWarnings = TRUE)
    ...
    for (group2 in groups2){
      ...
      if(group1 != group2){
        ...
        for (item in de_celltypes) {
          ...
          de_results <- FindMarkers(de_subset, group1, group2, logfc.threshold = 0, min.pct=0, min.cells.group=10, test.use="MAST",latent.vars=covars)
          
          write.csv(de_results[order(de_results$avg_log2FC, decreasing = T),],
                    file = paste0("Differential_Expression/DE_Results[1]/",data_type,"_",group1, "_",group2,"_",item,"_DE_",paste(covars, collapse='_'),".csv"))
...
volcano_plot <- function(markers,data_type,group1,group2,item){
  ...
  p1
  pdf(file = paste0("Differential_Expression/DE_Volcanos[1]/",data_type,"_",group1, "_",group2,"_",item,"_VOLCANO_",paste(covars, collapse='_'),".pdf"))
  plot(p1)
  dev.off()
