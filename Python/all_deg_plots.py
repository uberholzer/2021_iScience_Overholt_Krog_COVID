...
res_dir = wd + '/Differential_Expression/DE_Plots[1_6]/All_DEGs/'
...
def gets_degs(prefix,condition,celltypes):
    ...
    for ctype in celltypes:
        data = pd.read_csv(DE_dir+prefix+condition+ctype+"_DE_nCount_RNA.csv",index_col=0)
...
def matches2dataframe(prefix,condition,celltypes,ordered_celltypes,comparison,sample_type,degs):
    ...
    combinedDF.to_csv(res_dir+sample_type + comparison+'DEG.csv')


# ## Function for making compartmental comparisons
def jaccard(BALF_dataset,PBMC_dataset,comparison):

    pbmc_markers = pd.read_csv(res_dir+PBMC_dataset+'_'+comparison+'_DEG.csv',index_col=0)
    balf_markers = pd.read_csv(res_dir+BALF_dataset+'_'+comparison+'_DEG.csv',index_col=0)
    ...
    int_df_up.to_csv(res_dir+'INT_DEG_UP_'+comparison+'_'+BALF_dataset+'_'+PBMC_dataset+'.csv',index=False)
    ...
    # Print the dataframe to a .csv
    int_df_down.to_csv(res_dir+'INT_DEG_DOWN_'+comparison+'_'+BALF_dataset+'_'+PBMC_dataset+'.csv',index=False)
...
def jaccard_ctrl(dataset1,dataset2,comparison):
    ...
    ds2_markers = pd.read_csv(res_dir+dataset2+'_'+comparison+'_DEG.csv',index_col=0)
    ds1_markers = pd.read_csv(res_dir+dataset1+'_'+comparison+'_DEG.csv',index_col=0)
    ...
    for ctype in overlap_celltypes:
        ...
        intersect_genes_co_up=list(set(ds1_list_up) & set(ds2_list_up))
        intersect_genes_contra=list(set(ds1_list_up) & set(ds2_list_down)) + list(set(ds1_list_down) & set(ds2_list_up))
        intersect_genes_co_down=list(set(ds1_list_down) & set(ds2_list_down))        
    ...
    int_df_up.to_csv(res_dir+'INT_DEG_UP_'+comparison+'_'+dataset1+'_'+dataset2+'.csv',index=False)
    ...
    # Print the dataframe to a .csv
    int_df_down.to_csv(res_dir+'INT_DEG_DOWN_'+comparison+'_'+dataset1+'_'+dataset2+'.csv',index=False)
...
def roseplot(df,sets,comparison):
    ...
    fig.update_layout(
        title=sets+' '+comparison+' All DEGs',
    ...
    fig.show()
    plotly.offline.plot(fig, filename=res_dir+'rose[1]_DE_'+sets+'_'+comparison+'.html')
    fig.write_image(res_dir+'rose[1]_DE_'+sets+'_'+comparison+'.pdf') # Must have Kaleido installed
...
def roseplot_ctrl(df,sets,comparison):
    ...
    fig.show()
    plotly.offline.plot(fig, filename=res_dir+'rose[6]_DE_'+sets+'_'+comparison+'.html')
    fig.write_image(res_dir+'rose[6]_DE_'+sets+'_'+comparison+'.pdf') # Must have Kaleido installed
...
#-------------------------------------------------------------------------------------------------------------------------------#    
# # All DEGs Intra-Compartmental Control Rose Plots [6]
